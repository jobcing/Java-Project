...
<body>
	<div id="content">
		...
		<div id="pitch">
			...
			<form action="./listCafe.jb" method="get">
				<%-- font size 를 13pt 변경 --%>
			...
			* 상호명을 입력하지않고 검색하면 전체 검색결과가 표시됨 *
			...
			<table width="100%" cellpadding="5" cellspacing="2"
				...
				<tbody>
					<%-- 해당 게시글이 없다면 표시 --%>
					<c:if test="${ cafePage.hasNoNotice() }">
						<tr>
							<td colspan="4">게시글이 없습니다.</td>
						</tr>
					</c:if>
					
					<%-- ArrayList<Cafe>를 받아와서 cafe변수로 설정 --%>
					<c:forEach var="cafe" items="${ cafePage.content }">
						<tr>
							<td align=center><span style="color:blue">${ cafe.score }</span></td>
							<td align=center>${ cafe.name }</td>
							<td>${ cafe.comment }</td>
							<td align=center>${ cafe.getNickname() }</td>
						</tr>
					</c:forEach>
				</tbody>
				...
				<tfoot>
					...
					<c:if test="${ cafePage.hasNotices() }">
						<tr align=center height="50">
							<td colspan="4">
								<c:if test="${ cafePage.startPage > 5 }">
									<a href="listCafe.jb?pageNo=${ cafePage.startPage - 5 }
													&station=${ station }&name=${ name }">[이전]</a>
								</c:if> 
								<c:forEach var="pNo" begin="${ cafePage.startPage }"
									end="${ cafePage.endPage }">
									<a href="listCafe.jb?pageNo=${ pNo }
													&station=${ station }&name=${ name }">[${ pNo }]</a>
								</c:forEach>
								<c:if test="${ cafePage.endPage < cafePage.totalPage }">
									<a href="listCafe.jb?pageNo=${ cafePage.startPage + 5 }
					...
					<tr height="50">
						<td align="right" colspan="4">
							<%-- 버튼을 클릭하면 해당 페이지로 이동 --%>
							<input type="button" value="한줄평 등록" onclick="location.href='writeCafe.jb'"/>
						</td>
					</tr>
				</tfoot>

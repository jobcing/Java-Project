			...
			* 상호명을 입력하지않고 검색하면 전체 검색결과가 표시됨 *
			...
			<table width="100%" cellpadding="5" cellspacing="2"
				...
				<tfoot>
					...
					<c:if test="${ barPage.hasNotices() }">
						<tr align=center height="50">
							<td colspan="4">
								<c:if test="${ barPage.startPage > 5 }">
									<a href="listBar.jb?pageNo=${ barPage.startPage - 5 }
													&station=${ station }&name=${ name }">[이전]</a>
								</c:if> 
								<c:forEach var="pNo" begin="${ barPage.startPage }"
									end="${ barPage.endPage }">
									<a href="listBar.jb?pageNo=${ pNo }
													&station=${ station }&name=${ name }">[${ pNo }]</a>
								</c:forEach>
								<c:if test="${ barPage.endPage < barPage.totalPage }">
									<a href="listBar.jb?pageNo=${ barPage.startPage + 5 }

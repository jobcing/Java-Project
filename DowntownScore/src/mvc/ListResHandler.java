...
public class ListResHandler implements CommandHandler{

	private RestaurantService restaurantService = new RestaurantService();
	private static final String FORM_VIEW = "./listRestaurant.jsp";
	...
	public String process(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		if(name.equalsIgnoreCase("") || name == null){ // 만약 사용자가 상호명을 입력하지 않았다면
			restaurantPage = restaurantService.getRestaurantPage(pageNo, station);
			scoreAvg = restaurantService.getAvg(station);
		} else{
			restaurantPage = restaurantService.getRestaurantPage(pageNo, station, name);
			scoreAvg = restaurantService.getAvg(station, name);
		}
		
		// 구한 평점 평균을 소수점 2자리까지만 표시한다.
		scoreAvg = Double.parseDouble(String.format("%.2f", scoreAvg));
		
		req.setAttribute("restaurantPage", restaurantPage);
		req.setAttribute("scoreAvg", scoreAvg);
		req.setAttribute("station", station);
		req.setAttribute("name", name);
		
		return "./listRestaurant.jsp";
	}
}
